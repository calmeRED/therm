...
# 支持的文件扩展名（按需修改）
SUPPORTED_EXTENSIONS = {'.py', '.js', '.ts', '.jsx', '.tsx', '.html', '.css', '.md', '.yaml', '.yml', '.json', '.txt'}

# 默认排除的目录（包括隐藏目录和常见构建目录）
EXCLUDE_DIRS = {
    '.git', '__pycache__', '.venv', 'venv', 'node_modules',
    '.idea', '.vscode', '.mypy_cache', '.pytest_cache',
    'build', 'dist', 'logs', '.DS_Store'
}
...
def main(output_file="project_code.md"):
    ...
    for dirpath, dirnames, filenames in os.walk(root, topdown=True):
        ...
        dirnames[:] = [d for d in dirnames if d not in EXCLUDE_DIRS and not d.startswith('.') and d not in ["archive"]]
